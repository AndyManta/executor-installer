...
wait_for_wallet_log_and_save() {
    ...
    while true; do
        address=$(journalctl -u executor-installer --no-pager -n 50 -r |
            grep '✅ Wallet loaded' |
            sed -n 's/.*\({.*\)/\1/p' |
...
run_executor_install() {
    ...
    sudo systemctl disable --now executor-installer.service 2>/dev/null
...
create_systemd_unit() {
    local unit_path="/etc/systemd/system/executor-installer.service"
...
EOF
    ...
    sudo systemctl daemon-reload
    sudo systemctl enable --now executor-installer
    systemctl is-active --quiet executor-installer && echo "🚀 Executor is running." || echo "❌ Executor failed to start."
}
...
configure_disabled_networks() {
    ...
    if confirm_prompt "🔄 To apply the changes, the Executor must be restarted. Restart now?"; then
        if sudo systemctl restart executor-installer; then
...
view_executor_logs() {
    if systemctl list-units --type=service --all | grep -q 'executor-installer.service'; then
        sudo journalctl -u executor-installer -f --no-pager --output=cat
    else
